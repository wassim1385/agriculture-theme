...
//Create the menus
function agriprod_menus() {
    register_nav_menus( array(
        'main-menu' => 'Main menu'
    ) );
}
//HOOK the menu function
add_action( 'init', 'agriprod_menus');

// Adds stylesheets & JS files
function agriprod_scripts() {
    //Normalize css
...
//HOOK
add_action( 'wp_enqueue_scripts', 'agriprod_scripts'); //This hook 'wp_enqueue_scripts' needs wp_head() function in header.php file to work

// Enable feature images and other stuff
function agriprod_setup() {
    //Register new image size
    add_image_size( 'square', 350, 350, true /* crop the image */ );
...
add_action( 'after_setup_theme', 'agriprod_setup' ); //When the theme is activated and ready!

// Creates a widget zone
function agriprod_widgets() {
...
add_action( 'widgets_init', 'agriprod_widgets' );
/*add_action( 'after_setup_theme', 'agriprod_widgets' );*/

/** Display the hero image on background in the front page **/
function agriprod_hero_image() {
...
add_action( 'init', 'agriprod_hero_image');
